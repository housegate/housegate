...
type Ack2Inputs struct {
	...
	// .SourceNode). ClaimBoundSource is the source_node the FSM recorded for the
	// statement (ClaimOutcome.BoundSource). ACK2 requires both to be non-empty and
	// equal: the local write and the sequenced claim must agree on the same
	// source, so a claim bound to a different (or blank) source can never be
	// acked. Both are surfaced explicitly so the gate does not silently trust a
	// blank source.
	PreparedSource   string
	ClaimBoundSource string
}
...
func Ack2Ready(in Ack2Inputs) (bool, string) {
	...
	if in.ClaimBoundSource == "" {
		return false, "RC has no bound source_node"
	}
	if in.PreparedSource != in.ClaimBoundSource {
