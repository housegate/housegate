...
// NativeMaterializer materializes captured ClickHouse Native ClientData packets
...
func (m NativeMaterializer) Materialize(ctx context.Context, schema payloadexec.TableSchema, st replay.PreparedStatement) ([]payloadexec.Row, error) {
	...
	rows, err := DecodeNativePayload(schema, m.Revision, st.Payload)
	if err != nil {
		return nil, err
	}
...
// DecodeNativePayload parses concatenated ClickHouse Native ClientData packets
...
// ValidateNativePayloadDecodable verifies that a Native payload can be decoded
...
func ValidateNativePayloadDecodable(schema payloadexec.TableSchema, revision int, payload []byte) error {
	if _, err := DecodeNativePayload(schema, revision, payload); err != nil {
