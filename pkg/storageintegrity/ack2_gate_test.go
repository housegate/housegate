...
// bound source_node must be non-empty and equal to the source that prepared the
// write. A blank or mismatched bound source withholds ACK2 even when both
// categories and the lifecycle are otherwise satisfied — the local write and
// the sequenced claim must agree on the same source.
