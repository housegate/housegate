...
// the Native lane's reach: these declarations all infer, but nativeColumnValue
// has no case for them (or, for Decimal, nativeBlockColumnPositions rejects the
// downcast type one stage earlier). Spec Q Phase 2 flips them one at a time;
// deleting a row here is the visible evidence a capability landed.
func TestNativeDecoderRejectsUndecodableInferableTypes(t *testing.T) {
	for _, tc := range []struct {
		...
		},
		{
			...
			wantErr: "unsupported column type *proto.ColDate32",
		},
		{
			declared: "UUID",
			column:   func() proto.ColInput { c := new(proto.ColUUID); c.Append(uuid.UUID{}); return c },
			wantErr:  "unsupported column type *proto.ColUUID",
		},
		{
			declared: "Decimal(18, 4)",
			column:   func() proto.ColInput { c := new(proto.ColDecimal64); c.Append(proto.Decimal64(12345)); return c },
			// Rejected one stage earlier than the others: ColAuto downcasts the
			// declared Decimal(18, 4) to a bare Decimal64, so the block/schema
			// type comparison fails before nativeColumnValue is consulted.
			wantErr: `type "Decimal64" does not match schema type "Decimal(18, 4)"`,
		},
		{
			...
			wantErr: "unsupported column type *proto.ColNullable[uint64]",
		},
	} {
