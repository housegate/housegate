...
func (p *Plugin) OnQuery(ctx context.Context, qctx *plugin.QueryContext) error {
	...
	if p.admissionConsumer != nil {
		...
		// write through the staged path. Relay must only drain/capture client
		// input and synthesize success after admission; forwarding the ordinary
		// Query/Data to upstream would double-write beside the staged unsafe
		// write.
...
// SuppressUpstreamExecution, so Relay withholds the ordinary Query/Data path and
// only drains input for capture. The completed admission is driven through the
// consumer HERE; success lets Relay synthesize EndOfStream to the client, while
// rejection/unavailability returns an error and no ordinary upstream INSERT has
...
func (p *Plugin) OnQueryInputComplete(ctx context.Context, qctx *plugin.QueryContext) {
	...
	// (and cleared) the admission before Relay synthesized success, so a consumed
	// admission is no longer in p.active. This completion observer only handles
	// the no-consumer case: record the completed admission as pending for a later
	// ConsumeAdmission caller.
	if p.admissionConsumer != nil {
		return
	}
	p.mu.Lock()
