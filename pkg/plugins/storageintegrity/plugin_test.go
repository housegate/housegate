...
func TestIngressConsumerReceivesCompletedAdmissionAndClearsPending(t *testing.T) {
	...
	// consumer, Relay has already withheld the ordinary upstream Query/Data path;
	// success returns nil so Relay can synthesize success to the client.
...
func TestIngressWithConsumerSuppressesOrdinaryUpstreamExecution(t *testing.T) {
	consumer := &recordingConsumer{}
	p, signer := newSignedIngressWithConfig(t, Config{AdmissionConsumer: consumer})
	sql := "INSERT INTO tenant.events FORMAT Native"
	...
	if !qctx.SuppressUpstreamExecution {
		t.Fatal("storage-integrity ingress with a consumer must suppress ordinary upstream execution")
	}
}
...
// end-of-input hook. In the configured-consumer path, Relay has suppressed the
// ordinary upstream Query/Data stream, so no ClickHouse INSERT can commit behind
// a failed admission.
