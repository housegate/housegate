...
// Relay reaches the terminating-block splice. The first plugin error is
// returned so Relay can reject the query lifecycle before the upstream sees the
// normal commit boundary; SuppressUpstreamExecution queries have already had
// their non-empty payload Data withheld.
