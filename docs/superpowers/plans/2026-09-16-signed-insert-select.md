...
- The authenticated schema object O contains complete opaque table IDs and column semantics but no logical database/table names. Every executable RP catalog joins the exact full pin/O scope to constructor-fixed, copied, trusted historical logical names with complete W/R/U coverage; names are never inferred from IDs, SQL, current metadata, default databases or scratch relations. Each immutable executor/core route serves one exact `(executor_profile_id, query_profile_id)` and one retained pin/O/name scope. Protected historical-name production, retention/recovery and independently trusted source/verifier agreement remain C1/D3/D4 enablement gates.
- Snapshot execution carries an explicit in-process-only invocation reference, first through B5's `VerifyRequest` and then through B4's `ExecutionRequest.SnapshotQueryReferenceID`. The exact reference is a selector supplied by the trusted lifecycle owner, never a wire field, authority token, `ReservationID` alias or derivation input. Historical authentication precedes current-use acquisition, which precedes artifact open; successful temporary-handle quiescence precedes invocation-lease close and output transfer. Unknown quiescence retains the tracked lease as live/uncertain for lifecycle recovery and yields no successful output.
- B5 uses public hardwired A2 historical signature verification, duplicate-preserving `[]QueryRoute` construction into private immutable exact-pair routes and a private payload lane. Its HG milestone emits only applied receipts after successful independent B4 execution, including valid mismatch and zero-row results; execution failures never become abort receipts. `match_source_root` requires exact full-state root, output count, output root and canonical affected-partition-after commitments. Authenticated abort transitions, candidate-byte/delta promotion proof and real AC/C1/C5 lifecycle wiring remain their existing cross-component gates.
