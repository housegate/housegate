...
- 2026-08-19 final review-driven completeness correction for Housegate Tasks 11/14/15: schema-bound touched partitions are derived and journaled even when pressure is disabled, because candidate-set integrity is independent of capacity gating. Nil and known-empty partition sets are distinct protocol states: non-nil empty survives clones/journal replay and a recovered zero-row pending statement receives a zero-capacity reservation handle so it can converge. Candidate-bearing records with unknown partitions remain journal version 0; observation alone cannot falsely upgrade them. Legacy touched-partition migration stages the durable snapshot first and publishes memory only after a successful save, so a transient failure is retried rather than skipped. Every recovered `HasPrepared` record is validated for exactly one candidate per touched partition before `ClaimUnknown` status convergence or any RC/cleanup path. Concurrent callers share both full-journal and runtime-restore attempt errors, with later independent retry, while any malformed/conflicting/persistence-failed `RestoreBatch` latches admission unavailable across ordinary refreshes until a complete successful batch restores the full durable projection.
