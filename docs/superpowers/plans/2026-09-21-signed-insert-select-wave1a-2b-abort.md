...
- **Task 5 (docs).** The docs review verified every identifier, code and message against the code (about sixty checks, including the pinned modules); its one finding — the v2 guard set was only cited by test names — was closed with one paragraph the controller verified directly. The docs also correct a stale "v11 version byte" claim to v12.
