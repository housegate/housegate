...
| --- | --- | --- |
| **A4.1a — HG canonical authority** | HG owns `NativeArtifactV1`/`NativeArtifactSetV1`, the strict external-envelope validation and canonical hash rules, and independent valid/malformed literal vectors without changing the A1 record or vectors. | Supplies the canonical identity portion of Step 3 and the vectors consumed by Steps 1 and 4. |
| **A4.1b — HG offline generator** | A bounded offline tool measures final artifact files and emits the external profile bundle plus separate provenance; it is not a runtime analyzer or native artifact-set member. | Supplies Step 3's build → measure → bundle ordering and Step 4's exact measured inputs. |
| **A4.1c — complete mirrored corpus and registered RED** | Freeze the complete RG corpus, copy it byte-for-byte to RC, register both real test targets, and demonstrate the named capability assertions fail against empty handlers. A missing file or uncompiled test is not RED evidence. | Exactly Step 1 and Step 2; it freezes expectations but qualifies no behavior. |
...
- [ ] **Step 5: Consolidate and record one implementation in each engine.** Use `feat(rewriter): analyze pinned snapshot queries`. Consolidate reviewed local checkpoints before A4.6 builds its final artifacts; after A4.6 passes, record both exact engine digests and the RP pin beside the shared corpus. This step creates reviewable local commits and does not itself publish, push or activate capability. Unsupported checked arithmetic remains refused until it has its own vectors; do not claim that ordinary ClickHouse casts detect overflow.
