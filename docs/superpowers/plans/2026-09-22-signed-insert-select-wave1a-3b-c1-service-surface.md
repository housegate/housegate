...
Recorded during subagent-driven execution (arbiter-core #37 Task 1, arbiter-proto #12 Task 2, arbiter #111 Task 3, #112 Task 4, #113 Task 5, #114 Task 6, #116 Task 7 docs). Each item is a ruling made against the plan text above; the merged code is authoritative where the two differ.

- **Tasks 1 and 2 ran in parallel.** The arbiter-core and arbiter-proto lanes share no file or interface, so the single-implementer rule (conflicts in one worktree) did not apply; Task 3 waited for both merges.
- **Task 1 (arbiter-core).** The activation payload could not be produced through the abort purpose's shared `JWSCommandPayload` (which carries a command hash), so `QueryProfileActivationPayloadV1` uses the disposition purpose's inline-payload strict re-encoding while keeping the abort file's function set. The review found the verifier accepted malleated signatures (high-S and raw-V variants of one valid token) because it checked only the length; the canonical-signature check the disposition verifier makes (65 bytes, V ∈ {27, 28}, low-S) was added, with same-shape foreign-purpose and foreign-version cases. The four older purposes share the malleability gap and are carried as package-wide cleanup.
- **Task 2 (arbiter-proto).** `ActivateQueryProfileRequest` lives in `consensus.proto` (importing `raftlog.proto`'s command would create a cycle); `TestConsensusAdminRPCSignatures` froze the sixth method; no descriptor baseline moved.
- **Task 3 (arbiter).** The brief's `capabilityUpdate` builder and direct `Params.AuthorityAddresses` writes were replaced by the existing consensus-update fixtures; the readiness view exposes `Readiness.Policy`; the "disabled policy" refusal case signs the enabled policy and clears `Enabled` on the wire record because arbiter-core refuses to sign a disabled one (the FSM's shape validation refuses it before the signature check); `serverTestAuthority` reuses the abort authority key (the consensus-admin tests generate a random key), and `newServerTestFSM` installs its address for every server test. The review found six current-version tamper fixtures still labelled v13 after the bump (they routed through the migration branch instead of the always-on validators); they moved to v14, the replay case signs with a distinct `iat`, and shape refusals lost a duplicated prefix.
- **Task 4 (arbiter).** `publishServerCandidate` seals the child manifest the way `pendingTransitionPublish` does rather than copying the promotion pipeline (74 lines; the found path still runs through real wire commands). Two follower assertions moved to `NotLeader`; the review found the integration pipeline test had lost per-node convergence in the move, so each integration node now retains its FSM and the helper asserts convergence in process while the wire half requires exactly one serving node and a `NotLeader` detail naming it. The activation binding's `CandidateSeq` discriminator gained its test; both bodies set `keeper_shard_id` explicitly; an authority activation reports its activation index as `commit_index` (the disposition lane's `read_index` stays 0 for it, by the lane convention).
- **Task 5 (arbiter).** `validCandidateOrigin` shares the `genesis`/`v2` arms with the use-origin validator and keeps `transition` separate; seven shipped tests needed forced edits (index-0 settlement fixtures moved to a real index; the obligation "unknown kind" restore case moved to an obligation no candidate names because the candidate validator runs first; the tag-25 assertion now expects the server-owned-seam reason; the bare `transition` origin gained its parent and block). The review added the candidate key/sequence agreement rule the obligation validator already had. Consequence recorded for operators: with the capability enabled no path advances the safe watermark (tag 10 refuses; `PublishCandidate` does not write it) — the lifecycle test publishes its successor before enabling, and the candidate lane advancing the watermark is deferred.
- **Task 6 (arbiter).** The task review approved with no Critical or Important finding but noted that `ApplyArtifactDisposition` echoed `reply_nonce` without requiring it; fix round 1 (`02bfa82`) made `reply_nonce` required and pre-validates version, shard and network after the default-off gate and before the leader check and every observer call, so a malformed request costs no leader round, observer call or proposal (four tests assert zero of each). The reply body became one helper shared with `GetArtifactDisposition`, and the settlement-kind constants moved to `fsm` (`SettlementKindCandidateTerminal`, `SettlementKindQueryTerminal`, `SettlementKindReservationReleased`) so `server` no longer keeps a copy. Two concerns were handed to the final review: arbiter-core's manifest round-trip re-nils an empty `PartitionRoots`/`ActiveParts`, so an empty-table candidate cannot be signed through `RegisterCandidate`/`PublishCandidate` today (1a-3b' vectors); the role vocabulary is duplicated between `server` and `config`.
- **Task 7 (docs).** Written against a controller-verified identifier list rather than the plan text; the implementer corrected three pre-existing sentences that Task 6 had made stale (`ResolveObligation` is a coordinator action, `CloseUse` no longer says the pipeline is unwired, a removed Apply refusal message). The review approved with one Minor (proto field order in a shorthand). Fix round 1 added the final-review consequences beside their paragraphs: the per-read barrier cost and trusted-caller requirement, the administrator-JWS token-age bound this build does not enforce, tag 23 advancing the disposition lane's `LastAppliedIndex` and being superseded by the first transition publication, the block-0 selector limitation, and the restore bounds; the controller re-grepped every identifier and quoted string against the rebased lane before the PR.
- **Final whole-branch review and follow-up (arbiter #115).** The review (0 Critical, 4 Important, 7 Minor) found that an authority activation left `CandidateSeq` at zero and so wedged every later transition publication in the continuity check (fixed: the check is gated on the `publication` kind, and a transition publication supersedes an authority activation once the capability is enabled); that tag 23 did not advance the disposition lane's `LastAppliedIndex`, so `GetQueryPolicy` attested a `read_index` older than its `commit_index` (fixed on the successful write path, with tests asserting `read_index >= commit_index`); that the administrator JWS on `ApplyArtifactDisposition` has no freshness bound because arbiter-core's verifier is timeless by design (carried to 1a-3b' as its first task: an age-enforcing `Authorize…` in arbiter-core and `artifact_disposition.max_token_age`; this build refuses to enable the capability); and that the unconditional barriers cost one replicated `LogBarrier` entry per read on the production listener, which the Tier-3 statement above understates (decision 3 stands; the docs now state the cost and the trusted-caller requirement; the two new reads already refused malformed requests before the barrier and a guard test now pins that order; coalescing or rate limiting is ops hardening). Minors fixed: the authority restore arm bounds its index and block, `SettlementObservationsRead` iterates deterministically, candidate restore validates the origin, one test pins the `server`/`config` role vocabularies together, the `read_index` tests assert the property. Carried: the block-0 selector (an optional field in 1a-3b'), the empty-table candidate that arbiter-core's manifest round-trip cannot sign (1a-3b' vectors must include the case), the pre-barrier network read on Apply (accepted: an immutable genesis parameter already disclosed by other RPCs).
- **Carried to later slices.** Canonical-signature checks for the four older authority purposes; the candidate publication lane advancing the safe watermark (or routing the promotion loop through candidates); `reader_id`/`reply_nonce` on the two read messages and the arbiter-core client/mirror/vectors (1a-3b'); publisher credentials for tags 26/27 and `SourceClaims.RecordSnapshotArtifactReady`; the registry observer and the allocator (1a-3c); the items listed under Deferred above.
