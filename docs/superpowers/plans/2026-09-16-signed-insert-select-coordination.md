...
Add service tests for unauthorized/wrong-role publisher/admin, spoofed PublisherID, a valid old promotion-purpose signature used on a new command, missing/invalid committed proof, failed Barrier and stale/follower reads. Require no proposal or authoritative positive/negative reply on these failures. Successful cases authenticate the actual endpoint/role and bind proof to the requested committed record; a fake `authorized=true` or direct FSM view is insufficient evidence. D4 repeats these cases through actual process transport.
...
- [ ] **Step 3: Implement transition verification and disabled policy state.** Implement new-query administrative authentication and configured authorization before the control-plane operator may propose a compatible pair. Reuse the existing authority JWS cryptographic pattern with distinct new-lane purpose/domain bindings to each command; never reinterpret the old promotion purpose, trust a declared role, or treat a user signature as admin authority. Derive sender/role from verified credentials and require configured publisher/admin permission. Require no active reservation, unsettled prior work or profile/schema transition. Copy the entire old ledger and call the existing manifest sealing/root functions; independently verify all unchanged fields. A profile change is not a fabricated INSERT and does not edit the old manifest. Publish only after required transition receipts and artifact readiness validate. The compatibility catalog remains runtime/config-owned; this adds no user-selectable profile or new admin RPC, and does not migrate legacy authentication globally.
