...
C1 also owns the concrete committed-proof validation and fixed endpoint/network/shard trust used by B1's four-argument terminal verifier. Together with C3, it must freeze and test a deterministic authenticated association from each supported durable `expectedReferenceID` class to its real owner and exact predecessor `expectedPin`; parsing a reservation/generation spelling yields only a lookup key, and an arbitrary caller string, opaque nonempty bytes, direct FSM view or `authorized=true` cannot prove ownership. Unrecognized, unbound or unsupported reference classes, missing trust, stale/follower reads, failed Barrier, invalid proof or an association for another reference/pin refuse release. This uses existing committed records and proof encoding; it adds no wire field, RPC, purpose string or retention duration.
...
For terminal release, pass the real committed verifier the journal-supplied expected reference and pin. Prove a valid terminal record for reference A cannot release reference B even when both share one pin; wrong network/shard, predecessor pin, reservation/generation, assigned block, statement/input/original-JWS or terminal child refuses. Test every supported reference-owner class and refuse unrecognized classes or missing fixed trust. These real integration tests are distinct from B1's fake-verifier consumer tests and remain required before actual terminal-proof authentication is claimed.
...
C3 owns the authenticated reservation/assignment/terminal-child and cleanup/challenge authorization behind terminal release. With C1 it binds the expected reference owner to the exact predecessor pin, fencing generation, assigned block, statement/input/original compact-JWS hash and the applied or aborted child; a valid terminal for another reference on the same pin, another generation/block/child or unresolved source write is insufficient. Release authorization is per ownership class: query terminal does not release provisional publication ownership, and scheduling completion does not release the only remaining challenge/cleanup protection. The real verifier must preserve those independent obligations across restart and fail closed for unknown owner classes; it does not change the opaque proof bytes or public `Release` signature.

- [ ] **Step 4: Verify reconciliation, exact terminal release, safe publication and migration.** Lose Submit's response, replay the Raft log and recover the same identity/block. Abort during unknown prepare must retain ownership until the source settles or is durably quarantined/fenced. Drive C1's real verifier with the exact expected reference/pin and prove mismatch for another reference sharing the pin, wrong predecessor/reservation/generation/block/statement/input/original-JWS/child, missing trust and unrecognized owner class; no case may release ownership or objects. After authorized release of one ownership, every old generation is refused by claim/scan/promotion paths while other references and challenge/cleanup ownership survive restart and continue protecting artifacts. Persist remaining cleanup and retention debt across restart; the next block may proceed only after authoritative fencing/publication, while challenge artifacts remain retained. Test unauthenticated caller, wrong role, spoofed SourceNode, wrong-purpose admin signature and stale/follower status reads through service entry points before claiming this task complete. Run existing three-way, promotion, cleanup, seal and snapshot suites as regressions.
