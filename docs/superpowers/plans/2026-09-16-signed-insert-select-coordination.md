...
C1 owns the concrete AR in-process adapter for AC's local [`ArtifactPublicationControl`](../specs/2026-09-16-signed-insert-select-schema-semantics-design.md#publication-control-injection). Its constructor binds fixed network/shard, authenticated authority/leader-Barrier committed reader, current schema-authority verifier, independent configured publisher credential/authorization and the existing ready submitter; missing wiring refuses construction. Lookup verifies C1's existing committed-proof bytes and returns the original complete association for ready-but-unpublished or conflicting state, with `(nil, nil)` reserved for fresh authenticated absence. Record signs/submits only the existing readiness domain and verifies the committed reply; a lost response reconciles through lookup. Object fetch, proof/role/crypto, publisher signing and RPC work remain outside FSM apply and long-held locks; apply handles only deterministic committed identities/idempotence/conflicts. No new RPC or canonical record is added.

C1 also owns the concrete committed-proof validation and fixed endpoint/network/shard trust used by B1's four-argument terminal verifier. It constructs the evaluator with the same fixed `ArtifactOwnerRegistry` read view and applies the complete publication/replay/challenge origin and policy rules in the addendum's [local reference ownership](../specs/2026-09-16-signed-insert-select-schema-semantics-design.md#local-reference-ownership). Together with C2/C3 it authenticates every class's natural owner and exact predecessor `expectedPin`; grammar parsing and J/A are lookup identity only, while an arbitrary caller string, opaque nonempty bytes, direct FSM view or `authorized=true` proves nothing. Unrecognized, unbound or unsupported classes, wrong configured principal/namespace, missing trust, stale/follower reads, failed Barrier, invalid proof or another reference/pin refuse release. This uses existing committed records/proof encoding and the protected local registry; it adds no wire field, RPC, purpose string or retention duration.

Publication closure is not satisfied by readiness, fresh absence or a local cancelled label. C1/D3 must identify and implement a protected sealed-candidate lifecycle plus submission/publication exclusion that settles unknown proposals and rules out late readiness/publication commits across restart/leadership, then transfer to authenticated continuing retention. Current AR request-context cancellation stops only the RPC wait while ApplyFuture continues, and publication apply has no candidate cancellation fence. Challenge closure likewise requires concrete authenticated policy/horizon/debt authority with a monotone disposition or enforced replacement-admission fence: current Finality, LastMergeable and cleanup absence are not artifact-horizon authority, while OpenChallenge omits Safe/Rejected. These remain required implementation capabilities, not existing records or completed finite reclamation. The positive publication plus durable successor-retention branch remains feasible. If an existing authority seam cannot prove either missing predicate without a new replicated/public contract, return a bounded proposal before changing that contract; retain/resume/quarantine meanwhile rather than claiming cancellation or reclamation.
...
C3 owns the authenticated reservation/assignment/terminal-child and cleanup/challenge authorization for the addendum's `accepted` class and query-origin `challenge` ownership. It registers/retains accepted ownership during accepted-work recovery before execution, binding the exact predecessor pin, original grant/fence, assigned block/statement/root, input/read roots, original compact-JWS hash, source assignment, profiles and any claim. With C1 it authorizes release only for the exact applied/aborted child after publication/output or abort invariants, settled/quarantined writes, local-operation closure and durable replay/cleanup/challenge successors. A valid terminal for another acquisition on the same pin, another generation/block/child or unresolved source write is insufficient. Query terminal does not release publication ownership, and scheduling completion does not release the last challenge/cleanup protection. Finality/LastMergeable, observed cleanup absence and an OpenChallenge path that omits Safe/Rejected cannot serve as the required horizon/debt disposition; until C1/C3 implements a monotone authority or replacement fence, challenge release refuses. The real evaluator preserves these obligations across restart and fails closed for unknown classes; it changes neither opaque proof bytes nor public `Release`.

- [ ] **Step 4: Verify reconciliation, exact terminal release, safe publication and migration.** Lose Submit's response, replay the Raft log and recover the same identity/block; establish accepted ownership before resuming execution and never mint a second acquisition for the singleton. Abort during unknown prepare must retain ownership until the source settles or is durably quarantined/fenced. Drive C1's real evaluator with the exact expected reference/pin and protected registry view; prove mismatch for another reference/acquisition sharing the pin, wrong namespace/principal/predecessor/reservation/generation/block/statement/input/original-JWS/child, missing trust and unknown/unbound class. Crash every challenge-transfer boundary and prove the last scheduling owner is not spent before the exact successor is durably live; an already-spent successor cannot count as protection. No failure may release ownership or objects. After authorized release of one ownership, every old generation is refused by claim/scan/promotion paths while other references and challenge/cleanup ownership survive restart and continue protecting artifacts. Persist remaining cleanup and retention debt across restart; without authenticated monotone horizon/debt authority, challenge release remains refused rather than advertised as complete. Test unauthenticated caller, wrong role, spoofed SourceNode, wrong-purpose admin signature and stale/follower status reads through service entry points before claiming this task complete. Run existing three-way, promotion, cleanup, seal and snapshot suites as regressions.
...
- [ ] **Step 1: Add source-stage order, owner-lifecycle and fraud tests.** Inject source executor, journal, pressure, protected source owner capability and ClickHouse ports. Require no unsafe write before sequencer acceptance and exact partition capacity. Derive replay WORKER from verified/configured source credentials and bind it to C3's assignment; spoofed SourceNode/process labels, verifier privilege and another principal/namespace refuse. Tests compare actual named candidate bytes through the scanner, not just fake output hashes:
...
- [ ] **Step 3: Implement a separate query journal and reuse exact candidate machinery.** Through C5's constructor-bound source lifecycle capability, register/Retain this exact canonical `replay` acquisition before restore/execution, and recover same-live-operation retries to the same A. Call B4's `Executor.Prepare` once for the source and own its `PreparedQueryExecution`; track the read/scratch/output/cache/unsafe-submit handles under that acquisition. Read the exact globally ordered rows from `Output.OpenRows()`; copy/fsync a durable cache bound to input root, pin, output count/root and global IDs, then persist that binding before `Close` or any unsafe work. Reopen the durable cache for staging and derive touched partitions via the shared authority. `PreparedQueryExecution.Close` closes one handle, never the acquisition; close the acquisition only after all handles and unknown writes join or are authoritatively reconciled. Do not call Replay and then repeat the SELECT to recover discarded rows. Reserve capacity before **any** unsafe table write/candidate creation/claim. Scratch restore/evaluation has its own limits and semaphore, not the payload's known-byte budget.
