...
7. 全部验证成功后，workflow 才把仅限 tap 的 fine-grained PAT 交给 authenticated checkout，且不把 credential 持久化到 Git 配置；重新确认 tap revision 未变化后，publish step 通过临时 credential helper 推送并立即清理。
...
## 一次性 fine-grained PAT 配置

GitHub 的内置 `GITHUB_TOKEN` 不能写另一个仓库，因此使用专用 automation user 创建 fine-grained personal access token：

1. Resource owner 选择 `housegate`。
2. Repository access 选择 `Only select repositories`，且只选择 `homebrew-housegate`。
3. Repository permissions 只授予 `Contents: Read and write`，并设置明确的过期时间。
4. 如果 organization 要求审批 fine-grained PAT，由管理员批准该 token。
5. 在 `housegate/housegate` Actions secrets 中添加 token，名称为 `HOMEBREW_SYNC_TOKEN`。
6. 如果 tap 的 `main` 有 ruleset/branch protection，需要允许 token owner 直接 push；否则应将 `Publish formula` 改成 PR 模式，并为 token 增加 `Pull requests: Read and write`。

fine-grained PAT 可以限制到单个 repository 和 `Contents` 权限，但仍绑定创建它的用户并受过期策略影响；应使用专用 automation user，并在到期前轮换 secret：[GITHUB_TOKEN scope](https://docs.github.com/en/actions/concepts/security/github_token)、[Managing personal access tokens](https://docs.github.com/en/authentication/keeping-your-account-and-data-secure/managing-your-personal-access-tokens)。

`Sync Homebrew` 也支持手动 `workflow_dispatch` backfill。合并并配置 `HOMEBREW_SYNC_TOKEN` 后，可在 Actions 中运行 `Sync Homebrew`，输入 `v0.6.0`；workflow 会从该 exact release 的 `SHA256SUMS` 解析两平台 checksum，并走与正式 release 相同的验证和发布路径。
