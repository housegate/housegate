...
### Execution record and additional sub-project 5 gates (2026-09-26)

- Pins as executed: housegate `v0.15.0` (`04fb8c0`), arbiter-core `v0.10.0` (`755f094`), arbiter-proto `v0.8.0` (`de2cb91`), sentio-core `063059e` (`v0.0.0-20260926055138-063059e5277f`, sentioxyz/sentio-core#500). sentio-node: sentioxyz/sentio-node#186.
- Changes review added beyond this plan's text: every registry-recorded key (live Pending, Refused, Active, Retiring or Purging) is answered from the registry whatever the chain-state mirror says — an empty or stale mirror can no longer turn a governed table Ordinary; compensation covers user databases only, runs only while the registry is enabled, only for `CreatedBlock >= activation_block`, at most 16 declarations per pass; the storage-node RPC always gets the table-state source on an SI node (source-pinned); reconciler stats are exported (`sentio_node_storage_integrity_reconciler_tables{state}`, `…_table_failures{table}`, `…_unknown_tables`, `…_leftover_drops`) — a table stuck in `waiting_quiescence`/`purging` or with rising failures is a stuck purge.
- **Gate G1 — processor/driver tables (product decision required before enabling the registry on an SI indexer that hosts processor databases).** Tables `database_registry.Server.EnsureTable` creates after activation are recorded Pending by the arbiter and are never declared (the driver bypasses the commitgate Observer; compensation excludes non-user databases). HouseGate's `sitablestate` then refuses reads and writes to them with the retryable 733 (driver sessions are not exempt), so processor indexing stalls. Options: (a) the arbiter records non-user tables as ungoverned (the table state would then also need the database type to stop default-denying unrecorded processor tables); (b) declare processor tables and have the driver sign through an agent; (c) keep processor databases off the SI indexer.
- **Gate G2 — do not DROP a genesis table** until a follow-up lets startup skip genesis ids whose live incarnation is not genesis-origin (or is Purged): `snode.table_ids` is loaded before the follower answers, so after a genesis table is purged a restart fails to derive its schema (`clickhouse` source) or re-creates its `hg_*` tables every start (`network_state` source), and removing the id changes `schema_root`.
- Plan A carry-overs: upgrade every arbiter voter before any data-plane node; an evicted source SNode rejoins only with a wiped state directory; the data-plane client's default 4 MB gRPC receive limit vs. the ever-growing registry snapshot (follow-up task).
- Notes: a table created in a brand-new database can read Ordinary for up to one 1 s refresh on sub-second devnets (spec step 2; real chains close it through receipt polling); legacy intake-journal records without touched partitions fail HouseGate recovery at startup with a misleading "purged" message (all v1 records carry them).
