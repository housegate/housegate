...
- [ ] **Step 1: Write receipt/refusal and dispatch tests.** Assert a valid result with wrong source root returns a signed `match_source_root=false` receipt; invalid signature/unpublished pin/missing part/unsupported profile/stream failure returns no attestation. Require authenticated original-policy validation before dispatch. Historical replay uses its retained executor after current activation changes; old profile JSON without an executable route, a substituted backend/endpoint, an unknown pair or a missing route refuses without calling any executor or producing a receipt. A legacy request invokes only the legacy executor.
