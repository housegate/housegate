...
HG root replay owns the strict canonical artifact/outer records and projection validation; auth owns the pure schema-certificate payload plus the existing compact ES256K signing/verification convention. B1 fetches bounded exact outer bytes, requires byte-identical strict re-encoding, verifies inner and outer digests, artifact-set root, exact S/projection, certificate purpose/signature and dedicated network/shard-scoped schema-authority role before readiness. The authoritative fields, digest order and no-cycle rule are in the [schema-semantics addendum](../specs/2026-09-16-signed-insert-select-schema-semantics-design.md). Publisher authorization remains separate and occurs only after certificate/object/part validation.
...
- [ ] **Step 3: Implement publish-before-safe readiness and idempotent durable references.** Export exact selected candidate parts and the exact authenticated schema object O under content-addressed keys. `Publish` performs bounded strict object/certificate/digest/S/projection checks, validates candidate manifest/content and durably retains provisional publication ownership before readiness, without requiring published-safe membership; this breaks the readiness/publication dependency cycle. Validate complete upload and current schema-authority scope before the separately authenticated publisher may acknowledge readiness; AR's final publication gate consumes that acknowledgement. Only a separately authenticated published-safe pin and its committed exact readiness association authorize query `Retain`, `Open` and `FetchPart`; readiness or a certificate alone does not. Never delete old parts or O merely because a MergeTree merge made them inactive locally. Use atomic write/fsync/rename and a durable reference journal; account for provisional publication, reserve, accepted block, replay and challenge references independently. Reconcile abandoned provisional uploads against control-plane publication/cancellation authority before GC; do not free them merely on uploader disconnect.
...
- [ ] **Step 4: Run retention fault injection and cold fetch tests.** Crash between O/part upload, fsync, readiness, retain and release; reconstruct from disk, and verify no referenced object or original certificate disappears. Require a changed hint to work only for identical authenticated bytes, and preserve the exact O through authority rotation for historical reads. Run AC's dataplane and snode tests and preserve existing safe publication behavior while the feature is off.
...
- [ ] **Step 3: Implement authenticated restore and the restricted handle.** Verify the D1 publication record, committed exact O/readiness association, schema/root/profile identity, complete legacy projection and all descriptor entries before trusting bytes. Preserve the typed object on the handle for B4; current live metadata never substitutes. Download all active parts of R, not predicate-selected partitions. Validate per-part physical hash, rows, schema and partition; scan with the existing shared row authority and fold every part into its partition commitment. Use an immutable imported copy or a proven immutable hardlink/reflink; do not attach mutable live part paths. Scratch names are random operation-local transport names excluded from commitments.
