...
- [ ] **Step 3: Implement publish-before-safe readiness and idempotent durable references.** Export exact selected candidate parts and the exact authenticated schema object O under content-addressed keys. `Publish` performs bounded strict object/certificate/digest/S/projection checks, validates candidate manifest/content and durably retains provisional publication ownership before readiness, without requiring published-safe membership; this breaks the readiness/publication dependency cycle. For first commitment, validate complete upload and the fixed schema-authority scope before the separately authenticated publisher may acknowledge readiness; AR's final publication gate consumes that acknowledgement. If authoritative lookup instead proves the identical root was already committed, revalidate O/roots/S/projection and publisher authentication, then resume publication from that original evidence without applying the current authority map again. Conflicting or uncommitted roots never use this branch. Only a separately authenticated published-safe pin and its committed exact readiness association authorize query `Retain`, `Open` and `FetchPart`; readiness or a certificate alone does not. Never delete old parts or O merely because a MergeTree merge made them inactive locally. Use atomic write/fsync/rename and a durable reference journal; account for provisional publication, reserve, accepted block, replay and challenge references independently. Reconcile abandoned provisional uploads against control-plane publication/cancellation authority before GC; do not free them merely on uploader disconnect.
