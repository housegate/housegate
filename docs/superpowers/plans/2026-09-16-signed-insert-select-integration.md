...
query-auth signer signs final Query body -> persist ForwardUnknown intent -> serialized forward gate
if still live: transfer generation to upstream lifecycle -> write Query exactly once
...
The relay marks active ownership before starting preparation and runs a coordinator separate from its sole framed client reader. The reader publishes Cancel/EOF/protocol rejection directly into the generation's cancellation gate; it never waits for worker completion or a full event queue to make cancellation visible. Worker completion can only offer a detached result. Recheck generation/cancel state before applying it, before/after continuation, and at the serialized final forwarding transition. Journal `ForwardUnknown` before that transition, then check cancellation again; no gate is held during journal/RPC/FFI/hook/network I/O. The winning transition records forwarding ownership before the first upstream Query byte; cancellation observed first forbids every byte, while a write attempt that won is indeterminate until host status/release fencing reconciles it. After forwarding, preserve the same active generation for the ordinary upstream framed terminal response; do not begin a second query or locally synthesize success.

Before forwarding, Cancel/EOF cancels preparation and records durable cancel intent. `ReconcileCancel` runs with a service-owned bounded-attempt recovery context: authenticate lookup by persisted request identity, release the exact unconsumed draining/granted generation, or persist an absent-request cancellation tombstone through C2 so a delayed begin cannot recreate the reservation. Retry lookup after a concurrent state change; a local context error, empty response or one negative lookup is not proof that an in-flight acquire cannot commit. A late grant/native result is recorded for reconciliation but cannot forward or mutate live query state. After forwarding/Submit becomes indeterminate, retain the original envelope and reconcile host/sequencer identity before release; consumed/accepted work belongs to durable C4/C3 resolution. Reconnect/restart must not resurrect a canceled generation or re-sign it. Completion/abort hooks and client terminal writes have one coordinator owner; never race cleanup with unfinished continuation hooks.
...
before Run/Submit: persist intent without holding gate; serialize cancellation versus submit launch
...
Carry the same cancellation generation into C4's pre-Submit launch guard: a live check merely before spawning `Run` is insufficient because cancellation can win while signed input is being journaled. Gate the actual first Submit attempt after durable intent, with no network I/O under the gate. If cancellation wins, do not invoke the sequencer and reconcile/release through C2; if launch wins, preserve `SubmitUnknown` until authenticated lookup resolves it, even when the client context is canceled. After acceptance use the service-owned recovery context. A late worker can report recovery facts but cannot write another terminal packet or clear a newer query generation.

- [ ] **Step 4: Run all relay/plugin lifecycle regressions and real-client cases.** `bazel test //pkg/proxy:proxy_test //pkg/plugin:plugin_test //pkg/plugins/snapshotquery:snapshotquery_test //pkg/plugins/sisnapshotquery:sisnapshotquery_test //pkg/storageintegrity:storageintegrity_test`. Include routed/trusted/forward-pivot sessions, compression/chunk negotiation and revision `54470`; assert one reader, no ordinary upstream Query/Data, no double completion and correct next-query behavior after supported terminal boundaries. Exercise agent drain-blocked acquire Cancel/EOF, late grant, canceled native finalization, one-shot query-auth continuation, host pre-Submit fsync/launch races and restart after lost acquire/release/Submit responses. Require no agent upstream Query and no host Submit whenever pre-forward cancellation wins; after forwarding wins require reconciled ownership instead. D4 repeats this through real source/verifier/Arbiter processes.
...
Include A10/A11 agent pre-submit cases explicitly: hold a prior write before safe publication so acquire drains, then send Cancel/EOF and assert immediate reader observation; deliver a late grant or lose the grant response, restart the agent, and require durable request-identity cancellation/release with no upstream Query or host Submit. Race cancellation at finalization, continuation, forward intent and host Submit launch; record which serialized boundary won, exact hook counts and one reader, plus accepted/unknown reconciliation when cancellation loses. A blocked RPC test that only checks the final client error does not prove these forbidden side effects.
