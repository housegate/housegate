...
Include A10/A11 agent pre-submit cases explicitly: hold a prior write before safe publication so acquire drains, then send Cancel/EOF and assert immediate reader observation; deliver a late grant or lose the grant response, restart the agent, and require durable request-identity cancellation/release with no upstream Query or host Submit. Race cancellation at finalization, continuation, forward intent and host Submit launch; record which serialized boundary won, exact hook counts and one reader, plus accepted/unknown reconciliation when cancellation loses. A blocked RPC test that only checks the final client error does not prove these forbidden side effects. Repeat all four D1 launch-authorization crash windows on both real agent and host processes; assert intent-only recovery never launches after NotFound, durable authorization restricts retries to the same identity, and accepted/consumed evidence always retains recovery ownership.
