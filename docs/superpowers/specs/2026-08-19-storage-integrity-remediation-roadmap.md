...
2. **Peer-trusted, forwarded, maintenance and platform-operator sessions keep bypassing SI policy in v1**, and that becomes a recorded, tested decision rather than an accident (Spec I D6). Changing it means peer SQL — already rewritten by the originating proxy — would be re-rewritten, which the peer-trust design forbids. The mitigation is a documented network-isolation requirement plus a startup warning when SI tables are configured and an internal port is open.
