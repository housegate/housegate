...
**Import red line (the structural form of §4.3 red lines / §13 tripwire).** `fsm` imports only the root `arbiter` package, `accumulator`, `wire` (for the decoded Go command union), `authority` (the §9 command-hash functions and purpose/payload constants for wall-clock-free audit verification), `github.com/ethereum/go-ethereum/crypto` (secp256k1 recovery for the §6/§9 JWS checks), `housegate/housegate/pkg/replay` + `pkg/lthash`, and `hashicorp/raft` (for the `raft.FSM`/`raft.Log` interface types). `gen/pb` may appear only in `wire`'s imports — and the CI tripwire checks fsm's DIRECT imports (`go list -f '{{join .Imports "\n"}}' ./fsm`), since the transitive fsm→wire→pb path is the sanctioned design. Canonical hashing therefore cannot run over proto types by construction — the property is import-graph-checkable, and CI enforces it mechanically (§11).
...
- **Per-replica conjunction, quorum over passing replicas** (§7.3 check 3's "for each verifier in the quorum" reading). Replica r passes iff all three hold:
  ...
  2. for every partition p the block touches: `BaseRoot(p) ⊕ Σ(partition_new_part_sums[p] across the block's RCs) == r.Receipt.PartitionCommitmentsAfter[p]`, computed with `pkg/lthash` addition over raw 2048-byte accumulators (hex-decoded). **Erratum (2026-07-05 whole-branch review):** `BaseRoot(p)` is pinned by the block itself — the partition root recorded in `Manifests[header.PrevSafeSnapshotID]` (the base-design §7.3 "raw LtHash accumulator from the base manifest"), with the all-zero accumulator when that manifest is absent or lacks p. It is deliberately NOT the live `Partitions[p].BasePartitionRoot`, which a same-partition `RecordPromotionAck` advances: a live base would make the verdict depend on evidence-vs-ack ordering (breaking "evaluate early or late yields the same verdict") and would diverge from the frame the verifier actually replayed against (`ReplayJob.PrevSafeSnapshotID` = the header's). The live `Partitions` state remains the closure/CAS anchor for §9's `RecordPromotionAck` only;
...
| `ResolveChallenge` | verdict SAFE → the claim stands: statements resume at QuorumVerified, EXCEPT a statement whose candidate parts were all drained by acked promotions while under challenge, which resumes directly at Safe (erratum 2026-07-05: the consumed promotion cannot re-fire, so QuorumVerified would strand it); REJECTED → Rejected (v1 immediate adjudication, §7.5) |
