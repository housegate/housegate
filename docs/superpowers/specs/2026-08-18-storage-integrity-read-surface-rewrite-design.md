...
- housegate: `buildDynamicArgs` fills args; setting parsing (invalid mode → Exception); nil port + unsafe_latest → Exception; a rewriter-classified SI INSERT is rejected when the signed ingress lane is disabled and reaches ingress when it is enabled; docker `pkg/integration`: SI INSERT via staged path then `SELECT count()` returns rows in `unsafe_latest` immediately and in `safe` only after a fake promotion moves the partition (reuse `storage_promotion_mvp_test.go` fixtures).
