...
2. reads client packets: non-empty `Data` → fire `OnClientDataStrict` and buffer `Packet.Raw` (Relay owns the buffer bounded by `MaxPayloadBytes`; exceeding it → Exception + abort); `Cancel` → abort; the terminal empty block → fire `OnQueryInputCompleteStrict`. A client-side end-of-external-tables empty block is optional. When the first empty block precedes all payload, Relay uses a bounded, non-consuming packet-start lookahead: another packet means the first empty was the marker; no packet means it was the markerless terminator. This lets the strict plugin reject a zero-payload SI INSERT without waiting forever;
...
4. keeps the existing invariants: one query in flight; the client codec's `proto.Reader` is never replaced; forwarding uses `WriteRawPacket`, never `Splice`; the deferred buffer is released on every exit path. The deferred gate stays active until the real upstream terminal. Post-sample `Exception`, upstream read failure, and `EndOfStream` observed before the terminator write completes are fatal and order lifecycle hooks as abort → exactly one complete; only post-terminator `EndOfStream` fires success.
