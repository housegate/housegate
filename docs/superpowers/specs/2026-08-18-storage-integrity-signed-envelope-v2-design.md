...
2. reads client packets: non-empty `Data` → fire `OnClientDataStrict` and buffer `Packet.Raw` (Relay owns the buffer bounded by `MaxPayloadBytes`; exceeding it → Exception + abort); `Cancel` → abort; the terminal empty block → fire `OnQueryInputCompleteStrict`. A client-side end-of-external-tables empty block is optional. Classification is protocol-state-only, never clock-based: the first empty block before payload is always the external-tables marker; a first non-empty block means the marker was omitted and Relay later synthesizes it from the terminator. A standard zero-row producer sends marker + terminator and the strict plugin rejects it. A markerless zero-row stream is wire-identical to a lone marker, so Relay keeps waiting for a second empty block, `Cancel`, or EOF rather than guessing from elapsed time;
3. on success writes the (now token-carrying) Query upstream plus exactly one external-tables marker (replay the client's marker when present, otherwise synthesize it from the terminator), reads exactly one upstream response packet: `Data` (the upstream sample block — discard) or `Exception` (forward to client, abort); then writes every buffered raw Data packet via `WriteRawPacket` and the terminator, and resumes the ordinary upstream→client loop for the terminal `EndOfStream`/`Exception`;
4. keeps the existing invariants: one query in flight; the client codec's `proto.Reader` is never replaced; forwarding uses `WriteRawPacket`, never `Splice`; the deferred buffer is released on every exit path. The deferred gate stays active until the real upstream terminal. `Exception` before the marker write completes, post-sample `Exception`, upstream read failure, and `EndOfStream` whose decode-time snapshot precedes a completed terminator write are permanently fatal and order lifecycle hooks as abort → exactly one complete; a later writer completion cannot turn that observed EOS into success. Only post-terminator `EndOfStream` fires success. After writing the terminator, Relay continues observing client `Cancel`/EOF while the upstream terminal is pending; either aborts and disposes the upstream. A next Query that arrives only after terminal bytes reached the client is framed by the same sole codec reader, held until lifecycle completion, then handed back to the ordinary loop.
