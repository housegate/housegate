...
Non-goals: the DDL / schema-transition lane (Keeper-signed DDL, `ADD COLUMN`, …); §12.4 ledger-gated `hg_safe` merges (stays stopped, see §7); per-account fair scheduling of the throttle; partition-key expressions beyond the P1c freeze; migrating existing hand-created tables (devnet2 has none).

## 3. Decisions

**D1 — Table set and schema come from the schema source, not from local ClickHouse.** `EnsureProtocolTables` takes the `[]payloadexec.TableSchema` the role already resolves (`snode.Config.Tables` / verifier `cfg.Tables`, filled from `schema_source: network_state|chain|clickhouse`). Under `clickhouse` schema source the routine still runs but can only *verify* (it would be circular to create from what it reads); creation requires `network_state` or `chain`. Rejected: creating tables from the user's logical table observed on the source CH — replicas would depend on the source's local state, exactly the trust the layer removes.
...
Callers: `snode.NewRole` and `verifier.NewRole` (arbiter-core) call it during construction, before registering with the Arbiter; a periodic reconcile (default 60s) re-runs it so a table declared later on chain (07-30 chain schema source) is created on every node without a restart. sentio-node's `standalone.go` passes `NodeID` = its indexer node id and the schema source it already resolves. Reference binaries `cmd/arbiter-snode` / `cmd/arbiter-verifier` gain `--ensure-tables=verify|create` mirroring the schema source.
