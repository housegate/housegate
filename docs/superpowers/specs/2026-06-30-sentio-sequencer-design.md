...
The **Verifier** is a small separate process at the data plane (co-located with each ClickHouse replica) that embeds `pkg/replay.Verifier` + `chexec` + `payloadexec.Ed25519Signer` and a **new byte-side scanner** (net-new code using `pkg/lthash` primitives — `chexec` only materializes scratch tables, it does not scan existing on-disk parts by `_part`). It is a Sequencer client, not part of the Sequencer process. The Sequencer may embed `pkg/replay` too, but only for the optional challenge reference executor of base-spec §5.2.
