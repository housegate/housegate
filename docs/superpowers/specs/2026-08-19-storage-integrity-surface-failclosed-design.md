...
- Reserved-namespace addressing is refused for these sessions too. The rewrite bypass stays (they legitimately need un-rewritten SQL), but a narrow pre-check rejects any statement that names a reserved SI physical database or the reserved column. **It must not live inside `rewrite.Plugin`**: that plugin declares `RunOnForward() == false`, and `forward.Plugin` runs earlier and can set `IsForwarding`, so a guard placed there is skipped on exactly the forwarded sessions it needs to cover. It belongs in its own plugin that opts into both the forward and peer-trust filters and gates on the operator flags itself, leaving plain peer-trusted sessions untouched.
  ...
  The check is also **identifier-aware, not token-aware**, and this is not a detail: ClickHouse spells quoted identifiers with backticks and double quotes, so a scan that blanks those spans as if they were string literals lets ``FROM `hg_safe`.`t` `` through, while a scan that matches the bare token anywhere rejects an ordinary column named `hg_safe`. The rule is: erase string literals and comments, preserve and unquote identifiers, treat the reserved column as a violation anywhere, and treat a reserved database as a violation both in qualifier position **and** as a bare database target. That last clause is not optional — `TRUNCATE DATABASE hg_safe`, `DROP DATABASE hg_safe` and `USE hg_safe` put the reserved name in a bare path with no qualifier, so a qualifier-only rule fails open on the very attack §1b describes, and operator sessions skip the rewrite that would otherwise reject it. Bare-target detection must be anchored in grammar rather than in the presence of the word `DATABASE`: ClickHouse has a `database()` function, so a presence test flags the ordinary column in `SELECT database(), hg_safe FROM ordinary.t`. Distinguish the keyword from the function by whether `(` follows, skip `IF`/`NOT`/`EXISTS` between the keyword and the name, and scope any catch-all to statements whose leading keyword is a DDL verb so a `SELECT` can never trip it. Operators who need raw physical access use a direct ClickHouse connection, not the proxy — which is the existing posture for every other protocol-owned surface.
