...
- Reserved-namespace addressing is refused for these sessions too. The rewrite bypass stays (they legitimately need un-rewritten SQL), but a narrow pre-check rejects any statement that names a reserved SI physical database or the reserved column. **It must not live inside `rewrite.Plugin`**: that plugin declares `RunOnForward() == false`, and `forward.Plugin` runs earlier and can set `IsForwarding`, so a guard placed there is skipped on exactly the forwarded sessions it needs to cover. It belongs in its own plugin that opts into both the forward and peer-trust filters and gates on the operator flags itself, leaving plain peer-trusted sessions untouched.

  The check **refuses on mention and does not parse SQL**. Successive review rounds pushed an earlier design toward a hand-written ClickHouse lexer — quoted identifiers, qualifier versus bare database target, `database()` versus the `DATABASE` keyword, `IF [NOT] EXISTS`, `#`/`#!`/`//` and nested block comments, implicit `AS`-less aliases — and every gap in such a lexer is a bypass on sessions that already skip every other SI control. For a defense-in-depth guard over trusted roles the tradeoff runs the other way: a rare false positive costs one clear error with a stated remedy, a lexical gap costs the protection. So any statement from these sessions that mentions a reserved name outside a string literal or comment is refused.

  That leaves one lexical job whose failure direction is safe: erase `'…'` literals and every ClickHouse comment form (`--`, `#`, `#!`, `//`, and nested `/* … */`), refusing on an unterminated one; never erase backtick or double-quoted spans, since those are identifiers and erasing them is itself the bypass; refuse any quoted identifier containing a backslash, because ClickHouse applies string escape rules there and `` `hg\x5Fsafe` `` *is* `hg_safe` — decoding that escape set would rebuild the parser this design just removed; and match on identifier boundaries so `hg_safe_backup` does not trip. Under-stripping yields a false positive; over-stripping is the one case that can still hide a mention — a literal whose escapes are mishandled can run past its terminator — so the escape rules (`''`, `\'`, `\\`) are part of the contract and carry a regression test. The gain is that this is a single enumerable obligation rather than a whole parser's surface.

  The accepted false positive is stated rather than engineered away: an ordinary column named `hg_safe` is refused for these sessions. The guard exists only where SI tables are configured, the reserved names are protocol-owned, maintenance traffic here is DDL on fully-qualified physical names, and the error names the remedy — a direct ClickHouse connection for physical access.
