# Sentio Arbiter — Go Service Design

**Date:** 2026-06-30 **Status:** Proposed (v1) **Base:** [2026-06-22 storage integrity design](2026-06-22-storage-integrity-design.md) (the component called "Sentio Keeper" there is the **Sentio Arbiter** specified here) + [2026-06-10 multi-replica trust design](2026-06-10-multi-replica-trust-design.md) **Source of truth:** English version; regenerate the Chinese version from this file when changing semantics.

This document designs the **Sentio Arbiter**: the Go service that plays the §5.2 "Keeper" role of the storage-integrity design — sequencing signed statements, building L3 blocks, enforcing `statement_id` uniqueness, dispatching replay jobs, running the 2-of-3 three-way promotion check, issuing Keeper-signed promotion commands, and publishing safe-state manifests. It is implemented from scratch in Go on top of `hashicorp/raft`, not forked or ported from ClickHouse Keeper.

## 1. Positioning and the naming decision

The storage-integrity design calls this component "Keeper," but that name collides with the stock **ClickHouse Keeper** it runs alongside (§12.1 of that document spends a paragraph disambiguating the two), and the obvious alternative "Sequencer" collides with the **external L2's own sequencer**. This design therefore names the integrity-layer authority the **Arbiter** — the base spec's own word for the role (§4: "the integrity layer adds an external **content arbiter**") and the one that captures what distinguishes it from a plain L2 sequencer: it does not merely order writes, it **adjudicates which parts become `safe`** (the §7.3 three-way check). It still plays the ordering ("sequencer") role — it sequences signed statements into L3 blocks — and adds verification-orchestration and safe-state publication on top (§3.5). The two same-named-but-unrelated services to keep apart:

| | **ClickHouse Keeper** | **Sentio Arbiter** (this document) |
|---|---|---|
...
1. Executing user SQL on the hot path (the Arbiter dispatches replay; Verifiers execute).
...
The Arbiter is **control plane only** — it sequences, adjudicates, and signs. It never stores user parts, never executes user SQL on the hot path, and never talks to ClickHouse Keeper. All bytes live in the data plane.

**SNode is a role, not a separate daemon.** Throughout this document "SNode" denotes the storage-orchestration responsibilities — execute the source write, build `hg_promote`, run the Arbiter-signed `REPLACE PARTITION`, drop rejected parts, run `hg_unsafe` cleanup and safe-table audits, and carry the Arbiter-facing client (`RegisterRC`, subscribe promotions, ack). In deployment these are realized by the co-located Go process that already embeds the HouseGate proxy library (today's `sentio-node`); there is no separate SNode daemon. SNode and HouseGate are kept as distinct *roles* because they are different kinds of work — HouseGate is an on-path dataplane proxy (client↔ClickHouse native protocol) while the SNode role is off-path control-plane orchestration against ClickHouse storage — and they are best organized as distinct modules in that one process, not as more relay plugins. Both sit on the untrusted operator side (base-spec §4), so co-locating them changes nothing about the trust model: integrity comes from the Arbiter + the Verifier quorum regardless. The one boundary that must not be collapsed is operator-side (HouseGate/SNode/Verifier) vs the trusted, independent Arbiter control plane — folding the Arbiter into the operator process would remove the external, independent adjudicator §1 depends on.
...
```text
  ...
  └──────────────┘       │  │  · RC records · attestations  │            │
         ...
         │ ReplayJob      │  └───────────────────────────────┘            │
         │ (stream)      └─────────────────────────────────────────────┘
         │                              │ Arbiter-signed PromoteSafePartition
...
The Arbiter is a separate deployable (it is clustered control-plane infrastructure, not a per-connection proxy), reusing this repo's libraries and conventions:

```text
cmd/arbiter/main.go            # new binary: flags, config load, signal ctx, gRPC + raft bootstrap
pkg/arbiter/
...
NEW Go types (arbiter-go proto + pkg/arbiter): StatementEnvelopeV2 · RCRecord · ThreeWayVerdict ·
                        PartitionState · AnchorRef · PartRef · OpenL3Block · SeqGapSet · StatementState
external (to be created): github.com/housegate/arbiter-go/gen/pb   # wire contract, mirrors rewriter-go/gen/pb
```

The reuse boundary matters and was easy to overstate: `pkg/replay` today has `ReplayJob`, `ReplayAttestation`, `ExecutionReceipt`, `SafeSnapshotManifest`, `PartitionCommitment`, `PartManifestEntry`, and the `Statement` *replay projection* — it has **no** `StatementEnvelopeV2` (the signed envelope), **no** `RCRecord` (result claim), and **no** delta type. Those are new types this design introduces in the `arbiter-go` proto and `pkg/arbiter`. The `arbiter-go` module does not exist yet; it is created in P0 (§12).

The **Verifier** is a co-located data-plane component (with each ClickHouse replica) that embeds `pkg/replay.Verifier` + `chexec` + `payloadexec.Ed25519Signer` and a **new byte-side scanner** (net-new code using `pkg/lthash` primitives — `chexec` only materializes scratch tables, it does not scan existing on-disk parts by `_part`). It is a Arbiter client, distinct from the Arbiter process; it may run standalone or in the same node process as the HouseGate/SNode role, since quorum independence is required across nodes (>= 2 non-source verifiers), not within a process. The Arbiter may embed `pkg/replay` too, but only for the optional challenge reference executor of base-spec §5.2.

The `cmd/arbiter` binary follows the same lifecycle conventions as the rest of the repo: it loads `pkg/config`, threads a signal-cancelled context, exposes its gRPC listener through the `listenerRunner`/`serverListener` pattern used by `pkg/replicationproxy` (`Serve(ctx, ln) error`, graceful shutdown on context cancel), and logs via `pkg/log` `FromContext`/`Infow`.
...
```go
// pkg/arbiter/accumulator — statement_id uniqueness (§6)
...
// pkg/arbiter/fsm — the deterministic replicated state machine (a raft.FSM)
type FSM interface {
...
// pkg/arbiter/orchestrator — leader-only, side-effectful; proposes commands, never mutates State
...
// pkg/arbiter/raftnode — the consensus seam, so the raft library is swappable/testable
...
The three layers form a **commitment hierarchy**, and the Arbiter is the component that *authors* L3 and *anchors* it down to an external L2/L1 — it uses those chains, it does not run them. **L3** is Sentio's own hash-linked chain of signed write-statement blocks (authored by the Arbiter, §5.1); each L3 block's commitment is posted to an external **L2** (cheaper/faster finality), which settles to **L1** (Ethereum). Commitments flow *down* (L3 → L2 → L1); finality is inherited *up* (L1 → L2 → the anchored L3 block), and that finality gates promotion to `safe`.
...
flowchart TB
  ...
  subgraph SEQ["Sentio Arbiter · hashicorp/raft 3–5 nodes (trusted v1)"]
...
2. **② sequence** — HouseGate submits `StatementEnvelopeV2` to the Arbiter; the FSM assigns `statement_seq` and seals it into an L3 block. The source SNode optimistically writes `hg_unsafe` and registers its `RCRecord`.
...
7. **⑦ read** — `SELECT` hits `hg_safe` directly via HouseGate and **never enters the Arbiter**.
...
**Positioning.** In rollup terms the Arbiter is the *ordering authority* (the role a rollup calls the "sequencer" — the name we avoid here to prevent collision with the external L2's own sequencer); in v1 it additionally wears the *verifier-orchestration* (replay quorum + three-way check) and *settlement* (safe-state publication) hats, which the P5+ decomposition splits into the Arbiter vs the Verifier network (§1). It is optimistic-rollup-flavored (optimistic source execution + quorum replay + a P5+ challenge window), not validity-proof-based.

## 4. The Replicated State Machine

The Arbiter's source of truth is one Raft log and the derived state it feeds. This section defines the command alphabet, the FSM state, the `Apply`/`Snapshot`/`Restore` mapping, the determinism rules, and the lifecycle mapping.
...
The Arbiter cluster's own membership changes (adding/removing Raft nodes) use `hashicorp/raft`'s native configuration-change mechanism; that is a separate concern from the business commands above and must not be conflated with them.
...
type StatementState struct {
    Env            StatementEnvelopeV2          // NEW type (arbiter-go), carries user_jws + non-membership proof
    Seq            uint64
    Status         Status                       // §4.4
...
Reuse honesty: `ReplayJob`, `ReplayAttestation`, `ExecutionReceipt`, `SafeSnapshotManifest`, `PartitionCommitment`, `PartManifestEntry` are reused verbatim from `pkg/replay`, along with `Seal`/`Validate`. `StatementEnvelopeV2`, `RCRecord`, `ThreeWayVerdict`, `PartitionState`, `AnchorRef`, `PartRef`, `OpenL3Block`, and `SeqGapSet` are **new** types defined in the `arbiter-go` proto and `pkg/arbiter`. The accumulator is the only entirely new pure-Go algorithm component (§6).
...
**One canonicalization profile across the integrity layer.** Every root/commitment the Arbiter computes must go through `pkg/replay`'s canonicalization (`canonicalDigest`) with a new domain tag per commitment kind — never a second, parallel hash profile — because the entire safety argument rests on independent nodes deriving *identical* roots from the same evidence. `canonicalDigest` is currently unexported (`pkg/replay/hash.go`); **P0 exports it as `replay.CanonicalDigest`** (or adds a typed helper) so the Arbiter FSM, the Verifiers, and the §13/base-spec §13 audit all share one profile by construction rather than by convention.
...
The orchestrator detects a timeout, or a verifier reports a mismatch, and proposes `OpenChallenge` → `ChallengeReplay`. The challenge uses the **same three-way predicate** (base-spec §11): a challenger's independent replay reproduces `SourceClaimRoot` AND the partition-commitment check holds AND the byte-side check holds → the claim passes (`Safe`); any one failing → `Rejected` → unsafe parts dropped. v1's trusted Arbiter adjudicates immediately with no challenge window; the challenge window is the P5+ decentralized safety mechanism (base-spec §11 phased definition). This protocol-semantics rule is in v1 scope; the economic challenge/slashing parameters are deferred (base-spec §2 non-goal).

## 8. Promotion Command Issuance, hg_promote, and the Manifest

The Arbiter issues commands, serializes them, and records safe state; the actual ClickHouse SQL runs on the SNode.
...
| `PromoteSafePartition` / `UnsafeCleanup` | secp256k1 | single Arbiter authority, key shared across Raft nodes, leader-only use | `pkg/auth.RelaySigner` to sign + `EthValidator` to recover | one command-issuing trust root; SNodes already verify identities with an address allowlist |
...
1. **Arbiter level**: the FSM serializes promotions per `(table, partition_id)`, with each promotion's base being the CAS-verified safe snapshot at *publish time* (not at statement-execution time); it may batch several promotable statements into one `REPLACE PARTITION`.
2. **SNode level**: before building or publishing `hg_promote`, SNode takes a local `(table, partition_id)` publish lock and CAS-checks that the active `hg_safe` partition root still equals the command's `BasePartitionRoot`; if another promotion / mutation / safe-merge advanced it, SNode drops the shadow and rebuilds from the new base or waits for the Arbiter to rebase. The publish lock serializes only partition publication, not ordinary `SELECT` reads.
...
5. Operational discipline: Arbiter maintenance must be rolling, never a full stop.

Honest boundary: HA addresses single-node/leader failure; a full-group outage (majority down, bad deploy) still stops writes. This is an accepted v1 consequence of the centralized authority plus the engine split, and it is why base-spec §15 Q15 puts Arbiter HA on the critical path ahead of decentralization.

### 10.5 Relation to the §12.5 recovery paths

The base-spec §12.5 recovery paths (replay from the L3 stream, copy from a peer's `hg_safe`) rebuild `hg_safe` *after* the Arbiter recovers; they do not drain `hg_unsafe` while the Arbiter is still down. They are after-the-fact `hg_safe` recovery, not a during-outage write escape valve.
...
| **P3 harden safe serving (base-spec §13)** | maintain the Active read set, `EvictNode` on audit failure, orchestrate cross-node sampling | Arbiter side = **new** (membership / read set); scans on the data plane |
| **P4 expand language surface** | statement-level `schema_snapshot_id`; orchestrate `as_of_safe` + ordered safe+unsafe reads | mostly out of v1; small Arbiter changes |

Reuse vs new summary. **Reused as-is from `pkg/replay`:** `ReplayJob`, `ReplayAttestation`, `ExecutionReceipt`, `SafeSnapshotManifest`, `PartitionCommitment`, `PartManifestEntry`, the `Verifier`, and `Seal`/`Validate`. **Reused elsewhere:** `pkg/replay/chexec` + `payloadexec` (executor + ed25519 + `RowID`), `pkg/lthash` (accumulator), `pkg/auth` (secp256k1 key + `EthValidator` address recovery), and the `pkg/config`/`pkg/log`/`listenerRunner` conventions. **New:** the `StatementEnvelopeV2`/`RCRecord`/`PromoteSafePartition`/`ByteSideScanMsg` types and the whole `arbiter-go` proto module; `pkg/arbiter/{fsm, accumulator, orchestrator, server, raftnode}`; the authority promotion-signing payload; `cmd/arbiter`; the SNode-side promotion executor + byte-side scanner (data plane); and the `hashicorp/raft` wiring.
...
Must NOT appear:
- any path that makes the stock ClickHouse Keeper the promotion authority, or speaks ZooKeeper from the Arbiter;
...
7. **Backpressure signal**: the precise interface by which the Arbiter's promotion lag drives HouseGate admission throttling (§10.4 mitigation 3).
8. **Config schema**: enumerate the `pkg/config` `Arbiter` section fields (`node_id`, `raft.peers`, `raft.data_dir`, `raft.election_timeout`/`heartbeat`, `grpc_listen`, verifier pool / quorum sizing, authority key source, payload-store endpoint, `metrics_listen`) and which are required, mirroring how `replicationproxy_config.go` validates per-mode.

## 15. References

- [2026-06-22 storage integrity design](2026-06-22-storage-integrity-design.md) — the "Sentio Keeper" there is this Arbiter
...
- [pkg/replicationproxy](../../../pkg/replicationproxy) — listener/lifecycle conventions mirrored by `cmd/arbiter`
