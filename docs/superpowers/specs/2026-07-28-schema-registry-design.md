...
**Status:** Phase A implemented; Phase B/C proposed. **Base:** arbiter-core main (staged intake seam landed), sentio-node PR #163 (source SNode hosting), housegate PR #98 (`sicore.NativeCSVPayloadMaterializer` + `SchemaResolver`), sentio-core network state (`TableInfo{TableId, TableType}` — no column data today). **Parent designs:** [staged intake seam](2026-07-28-arbiter-snode-staged-intake-design.md) (the consumers), [P1c data plane](2026-07-06-arbiter-p1c-dataplane-design.md) decision 8 (`schema_root` as a genesis consensus param), [master arbiter design](2026-06-30-sentio-arbiter-design.md) §5.3 (schema barrier / `schema_snapshot_id`, the P2 lane this registry feeds), da.proto's hash-silent trust model (the pattern this design transplants).
...
| 2 | **On-chain storage is commitment-only; the event is the content channel.** In [compute-network-contracts](https://github.com/sentioxyz/compute-network-contracts) `Databases.sol`: append `schemaVersion uint64` + `schemaHash string` to `Types.Table` (tables live in `mapping(string => mapping(string => Types.Table)) _tables` — tail-appending to a mapping-value struct is UUPS-layout-safe, and the repo already practices slot discipline: `Database.__deprecated_* // slot preserved for UUPS`). New method `setTableSchema(databaseId, caller, tableId, version, schemaHash, schemaJson)` guarded by `createTable`'s exact double gate (`msg.sender == indexer signer` + `isDatabaseWriter(databaseId, caller)`); it stores only `(version, schemaHash)` and emits `TableSchemaSet(databaseId, tableId, version, schemaHash, schemaJson)` with the full column JSON in event data. `version` is strictly monotonic per table and does NOT reset across `deleteTable`/re-`createTable` (delete only flips `active`; the write-once `@<version>` state key must never be reused). | Storage stays ~2 slots/table; event data at ~8 gas/byte makes a few-KB schema cost tens of thousands of gas — cheap, and the event log becomes the permanent audit source the syncer (which already rebuilds state from events) mirrors into the `TableSchemas` collection. Precedent for content-in-pipeline: `ProcessorInfo.EntitySchema`/`EntitySchemaVersion`. |
...
| 8 | **Phasing:** **Phase A — Implemented:** CH-derived loader and consumers ([housegate#111](https://github.com/housegate/housegate/pull/111), [arbiter-core#6](https://github.com/sentioxyz/arbiter-core/pull/6), [arbiter#12](https://github.com/sentioxyz/arbiter/pull/12), [sentio-node#165](https://github.com/sentioxyz/sentio-node/pull/165)) replace column YAML with `table_ids`, read columns from local `system.columns`, and keep the root anchored to genesis. The CLI work lives in `arbiter` after the reference binaries split from `arbiter-core`. **Phase B** (this spec's main body): the four-layer registry + shared network-state loader + cross-check, replacing Phase A's CH source. **Phase C** (out of scope, P2): DDL barrier + snapshot minting on top. | A kills the YAML-drift problem now; B is a 4-repo change that lands behind it without conflicting (the loader seam is shared — only the source swaps); C is protocol work with its own spec. |
...
```
contract     Types.Table                 + schemaVersion uint64, schemaHash string
             setTableSchema(...) + event TableSchemaSet(dbId, tableId, version,
             schemaHash, schemaJson)     (compute-network-contracts: Databases.sol,
             Types.sol, IDatabases.sol, gen_bindings.sh → sentio-node bindings)
...
| Repo | Delivers | Phase |
|---|---|---|
| [compute-network-contracts](https://github.com/sentioxyz/compute-network-contracts) | `Types.Table` tail-append + `setTableSchema` + `TableSchemaSet` event (UUPS upgrade-safety review applies); regenerate bindings via `gen_bindings.sh` | B |
| sentio-core | state types + observer mirroring of schema content | B |
| sentio-node | Assembly switches `snode.Config.Tables` and `SchemaResolver` to one loaded set; config drops column YAML (A: `table_ids` + CH-derive; B: registry source) | A + B |
| housegate | A: `schemaregistry.Loader` seam + ClickHouse implementation. B: `pkg/registry.Table`/`pkg/network.TableInfo` widening and network-state implementation. | A + B |
| arbiter-core | A: exports canonical physical-table naming for consumers. B: reusable SNode/verifier consumption stays source-agnostic. | A + B |
| arbiter | Reference SNode/verifier binaries support `tables` XOR `table_ids`; derive-mode `-print-schema-root` connects to ClickHouse. | A |
