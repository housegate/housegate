...
2. **Creation into a governed name is allow-listed; everything else is refused (non-retryable).** "Governed" means `Lookup` returns Pending (including default deny) or Active; for data-carrying creation a Refused name counts as governed too, so a name cannot be dropped and refilled by `CREATE … AS SELECT` while it is still Refused. A `CREATE TABLE` of a governed name is allowed only when the SQL ClickHouse will run (the engine-normalised body) has no top-level `AS`, or a single top-level `AS` followed by a bare `[db.]table` schema copy and only non-data clauses (`ENGINE`, `ORDER BY`, `PARTITION BY`, `PRIMARY KEY`, `SAMPLE BY`, `TTL`, `SETTINGS`, `COMMENT`). `AS SELECT`/`WITH`/`FROM`/`(…)`, `AS` a table function, `CLONE AS`, and any statement the gate cannot lex with certainty are refused; a false positive on an unusual schema-only spelling is accepted. Any materialized view whose own name is governed is refused, as is one whose `TO` target is governed (the target is read from the statement header, not from the rewriter's accessed tables) or whose header cannot be read. Such statements would put rows into a Pending table's ordinary storage, which violates umbrella D10. The message tells the user to create the table first and then INSERT. Table engines that carry data by reference (`Buffer`, `Merge`, `Distributed`, `URL`, remote disks) are a rewriter-contract gap tracked by a separate security follow-up.
