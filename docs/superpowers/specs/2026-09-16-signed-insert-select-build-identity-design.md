...
Neither `record` nor `native_artifact_set` contains Q, a profile-file digest, a path, a release label or another profile identifier. This avoids a hash cycle; the `query_profile_id` in the outer entry is the independently recomputed lookup key.
