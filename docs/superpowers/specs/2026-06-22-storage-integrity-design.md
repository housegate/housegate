...
Keeper is the sequencer, validator, registry, attestation collector, and safe-state publisher. The Go architecture for this role — component breakdown, consensus/HA plan, signing scheme, and the `pkg/arbiter` package map — is specified in [2026-06-30 Sentio Arbiter design](2026-06-30-sentio-arbiter-design.md); this section defines the responsibilities, that design defines how they are built. **Naming:** that sub-spec names this role the **Sentio Arbiter** (from §4's "external content arbiter"), to emphasize that it *adjudicates* what becomes safe and to avoid confusion with both the unrelated ClickHouse Keeper and the external L2's own sequencer; throughout this document, "Keeper" in the integrity-layer sense refers to that same Arbiter role. "Keeper" is also used below for ClickHouse Keeper (the C++ ZooKeeper-compatible coordination service that backs `hg_unsafe` ReplicatedMergeTree); see §12.1 for that boundary — the two are unrelated despite the shared word.
...
**Storage-integrity rewrite bypass (recorded 2026-08-19, Spec I D6).** Peer-trusted and origin-side forwarding sessions bypass SQL rewrite because their SQL was already rewritten by the originating HouseGate; re-running it could double-prefix physical names. This is deliberate in v1 and is pinned by `TestPeerTrustedSessionBypassesStorageIntegrityRewrite`. The control is network isolation: `internal_listen` MUST be reachable only from trusted peer subnets, and HouseGate emits a startup warning when it is combined with configured SI tables. Maintenance and platform-operator sessions also retain the general rewrite bypass, but they carry raw SQL and therefore pass through the independent parser-free `sireserved` guard before forwarding. For those operator-flagged sessions the guard ignores comments but rejects `hg_safe` / `hg_unsafe` / `_hg_row_id` on identifier boundaries even inside string literals, any backslash-bearing literal or quoted identifier, every `{name:Identifier}` placeholder independent of `Query.Parameters` or `param_*` transport, and every known local-catalog object-carrier callable independent of arguments (`remote`/`remoteSecure`, `cluster`/`clusterAllReplicas`, `merge`, `loop`, `dictionary`, time-series/prometheus carriers, any `MergeTree*`, `distributed`, `buffer`, and `clickhouse`). This conservatism is required because ClickHouse accepts literal and constant-folded object identities; false positives name the direct-ClickHouse remedy. Ordinary peer/forward sessions without an operator flag remain outside this guard.
