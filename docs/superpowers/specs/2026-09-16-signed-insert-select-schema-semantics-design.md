...
The exact `ExecutionRequest.SnapshotQueryReferenceID string` field carries the JSON tag `json:"-"`, is copied at entry and is passed unmodified to both `AcquireQueryUse` and the unchanged `SnapshotReadStore.Open(ctx, pin, reads, referenceID)`. It is a selector for an existing protected registered acquisition, never an authority token, a `ReservationID` alias, a value minted or parsed-and-authorized by B4, or a durable close/release/GC operation. The trusted concrete admission adapter fixes registry/scope/principal/role and allowed owner class at construction; atomically against closing it authenticates current live admission and the complete accepted assignment, original JWS and roots, grant/fence, pin/O, profile pair, role/principal and required committed source claim. `HistoricalPolicy.VerifyReservation` and `decision.CheckJob` run first, acquisition runs second and artifact open runs third. Every dependency receives an owned deep copy of mutable invocation data.
...
For B4 current use, the `accepted` class authorizes only its current actual source lifecycle under the trusted source assignment; `replay` authorizes a separately admitted source or verifier execution with the exact configured role/principal/claim; and `challenge` protects obligations but requires a separate admitted replay acquisition to execute. `reservation`, `publication`, unknown, unbound and spent references cannot execute, and no class fallback is allowed. The outer lifecycle registers/retains the exact acquisition before handing its unmodified reference to B5/B4, while B4's invocation lease joins only that tracked invocation and never calls the durable `CloseUse` or local `Release`. A quiescence failure leaves that acquisition live/closing/uncertain for the concrete lifecycle owner; timeout, fixtures and GC cannot settle it.
