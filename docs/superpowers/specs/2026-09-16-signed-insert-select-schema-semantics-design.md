...
B5's HG consumer milestone is applied-only. Any B4 error, cancellation, malformed result, populated-result-plus-error, stream/EOF failure or required-close uncertainty returns a zero attestation with no fallback, retry, second acquisition, signer call or abort conversion. A successful result must be exactly `applied`, may contain zero rows, and carries B4/`ApplyRows`'s full sealed W/R/U `ComputedStateRoot`; affected partition commitments are additional affected-after evidence, not a complete manifest. `MatchSourceRoot` is true only when verifier full-state root equals `claim.ComputedStateRoot`, output count and output root equal their claim fields, and corresponding canonical affected-partition-after commitments match by exact table/partition identity, count and root independent of order or hints. Malformed/duplicate commitments refuse; a coherent difference signs an applied mismatch. The compound `SourceClaimRoot` is never compared to computed state. Candidate physical-byte equivalence, partition-delta proof and three-way promotion remain AC/AR/C5 gates, while an aborted receipt requires C3's separately authenticated committed abort transition.
