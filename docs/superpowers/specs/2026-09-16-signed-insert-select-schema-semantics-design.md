...
The exact constructor is `NewSnapshotArtifacts(backend ArtifactBackend, exporter PartExporter, published PublishedSnapshotSource, publication ArtifactPublicationControl, journalDir string, verifyTerminal func(context.Context, string, replay.SnapshotPin, []byte) error) (SnapshotArtifacts, error)`. Its verifier arguments are `(ctx, expectedReferenceID, expectedPin, terminalProof)`: B1 reads the immutable expected reference/pin binding from its durable journal and passes both values explicitly. C1/C3 own the trusted proof verifier and authenticated reference-owner/terminal authorization; a B1-local parser, nonempty opaque proof, caller string or mutable context/closure cannot establish ownership. Every callback consumer uses both expected values, and unrecognized, unbound or unsupported reference classes refuse release. Verification runs outside long-held journal/GC locks; immediately before durable mutation B1 rechecks the same live binding. Verifier failure or mismatch leaves the ownership and objects intact, and verifier success authorizes only that ownership release, not object GC. This changes no public `Release`, proof encoding, canonical/wire/hash record, RPC, purpose or retention duration.
