...
A5, D1 and D2 build the O-derived portion of RP catalogs only from the authenticated object's complete opaque table IDs and columns, revalidate exact S/projection/publication policy before final signing and host intake, and transport `generation` plus `default_expression` unchanged. O contains no logical database/table names. B4 and B5 therefore join the exact ordered O identities and column semantics to separately trusted, constructor-fixed historical logical names for the same complete pin/O scope before source or verifier execution; they never derive names from an opaque or dotted ID, current physical metadata, the target default database, submitted SQL or restored scratch coordinates. Engine `SUCCESS` validates supplied metadata/profile behavior but never proves publication or name provenance. B4 still supplies only the validated legacy projection to shared row/state assembly and preserves U in the complete ledger.
...
Logical names are immutable trusted configuration outside A/O and every wire/hash record:

```go
type LogicalTableName struct {
    TableID  string
    Database string
    Table    string
}
...
The B4 constructor validates and copies the complete full pin, canonical outer O digest and table slice into private exact-ID and exact `(Database, Table)` indexes. It rejects zero/incomplete pin scope, malformed digest, invalid UTF-8, NUL, empty or whitespace-only logical values, duplicate TableIDs, duplicate logical pairs and a table count above the existing schema bound. It preserves otherwise valid quoted-identifier values exactly, with no trimming, case folding, dot splitting or invented identifier grammar. Construction performs no artifact open or fetch and therefore does not claim that the configured names contain the authenticated complete W/R/U set. At invocation the configured pin and O digest must equal the job and authenticated historical decision before acquisition/Open; after history, current-use acquisition/Open and authenticated O validation, O's complete ordered table set must exactly equal the configured name set with no missing or extra ID, and every signed R descriptor's database/table/ID triple must match before analysis. All schema hashes, declaration order, columns, generation and expressions continue to come unchanged from O. Same-digest arbitrary labels prove nothing; absent trusted historical mapping refuses.

One B4 executor is frozen to one exact executor/query profile pair and one retained full pin/O/name scope. A concrete protected producer and journal must retain and recover the original explicit logical mapping with its pin/O association, and independently trusted source and verifier construction must consume the same immutable original mapping. D3 and C1 own and review that real producer, retention/recovery and agreement before enablement; fixtures and current metadata establish only orchestration. No logical name is added to `HistoricalDecision`, A, O, the job, signature, receipt or any canonical byte.

Current-use admission is a mandatory constructor-fixed port, separate from historical authentication:
...
type QueryUseLease interface {
    Close() error
}
```

The exact `ExecutionRequest.SnapshotQueryReferenceID string` field carries the JSON tag `json:"-"`, is copied at entry and is passed unmodified to both `AcquireQueryUse` and the unchanged `SnapshotReadStore.Open(ctx, pin, reads, referenceID)`. It is a selector for an existing protected registered acquisition, never an authority token, a `ReservationID` alias, a value minted or parsed-and-authorized by B4, or a durable close/release/GC operation. The trusted concrete admission adapter fixes registry/scope/principal/role and allowed owner class at construction; atomically against closing and relevant phase advancement it authenticates current live admission and the complete accepted assignment, original JWS and roots, grant/fence, pin/O, profile pair and role/principal. Its claim predicate is selected only by the trusted lifecycle owner from authenticated assignment/claim state and its protected recovery journal: initial execution of the exact accepted source assignment requires both `SourceClaim == nil` and `SourceClaimRoot == ""`; recovery of that same accepted operation preserves any already committed claim as a set-once exact complete claim/root binding and omission, one-sided input or conflict refuses; every verifier or separately admitted later source replay requires the complete exact committed applied claim and matching root. Caller absence, a nil field, reference spelling, role assertion or context never selects the phase, and an unknown claim outcome permits reconciliation only. `HistoricalPolicy.VerifyReservation` and `decision.CheckJob` run first, acquisition runs second and artifact open runs third. Every dependency receives an owned deep copy of mutable invocation data.

B5 accepts only `Verify(ctx context.Context, req VerifyRequest)`, where `VerifyRequest.Job replay.SnapshotQueryJob` and `VerifyRequest.ReferenceID string` both carry `json:"-"`. It copies and validates the complete request, rejects an empty or whitespace-only reference while preserving every valid byte, recomputes the read/input/statement roots from the original unmodified compact JWS, and calls the public hardwired `auth.VerifyStatementV3Signature` without today's freshness or allowlist rule. A private constructor-function argument may support ordering/failure fixtures, but the public `NewVerifier` exposes no signature callback, skip or mutable hook. Only after A2 identity verification does B5 call `HistoricalPolicy.VerifyReservation`, call `decision.CheckJob`, validate the complete applied C5 claim/hash/identity, and select its private immutable exact-pair route. B4 receives a separate owned job and the exact reference once, independently repeats the policy call and job check, checks the configured pin/O/name scope, acquires current use and only then calls `SnapshotReadStore.Open`; no caller-supplied decision bypass exists. B4/B5 use the retained original activation and certificate without substituting today's active policy or allowlist.

B5 constructs private routes from duplicate-preserving `[]QueryRoute`, rejects invalid/nil/typed-nil routes and duplicate exact `QueryProfileKey` values before map insertion, copies all entries, and keeps the legacy payload lane private. Query-only and legacy-only `CompositeExecutor` dispatchers are valid and an entirely empty dispatcher refuses, but query `NewVerifier` requires its dispatcher to contain at least one query route; a valid legacy-only composite remains usable only for the legacy lane. Mixed variants and stray query references refuse, and a missing exact pair never falls back to payload, current activation, another profile or a newly constructed executor. The requested exact-route lookup remains after signature, history and complete applied-claim validation. Each exact pair can name at most one immutable B4 executor and therefore one retained pin/O/name scope; trusted AC/C1 construction selects the correct retained core after authenticated scope resolution. The dispatcher supplies routing only, never history or current-use authority.
...
`ApplyRows` borrows `RowSource`; its caller owns and closes the stream even after errors. The legacy adapter closes streams it creates, while B4 closes every S-reading cursor, stream, scratch and read handle successfully before it may close the invocation lease, and closes that lease successfully before transferring the independent `CanonicalOutput`. Close is a quiescence contract, not best effort: any failed/unknown handle close or failed lease close joins the original and cleanup errors, yields no successful or partial output, and retains the tracked lease live/uncertain under its lifecycle owner for authoritative recovery. B4 never converts a failed close into a false release, drops the tracking record or manufactures terminal proof; an Open error must either finish all cleanup before return or leave the owner uncertain. When no artifact work or handle exists, the lease may close immediately. `Replay` closes the transferred output exactly once. Direct `ApplyRows` tests cover complete predecessor/configured-schema/legacy-projection consistency and ledger preservation. O1/O2 equal-projection and certificate substitution tests belong to B4 Prepare/Replay, which receives O. No O, policy argument or synthetic payload reference is added to legacy append APIs.

B5's HG consumer milestone is applied-only. Any B4 error, cancellation, malformed result, populated-result-plus-error, stream/EOF failure or required-close uncertainty returns a zero attestation with no fallback, retry, second acquisition, signer call or abort conversion. A successful result must be exactly `applied`, may contain zero rows, and carries B4/`ApplyRows`'s full sealed W/R/U `ComputedStateRoot`; affected partition commitments are additional affected-after evidence, not a complete manifest. `MatchSourceRoot` is true only when verifier full-state root equals `claim.ComputedStateRoot`, output count and output root equal their claim fields, and corresponding canonical affected-partition-after commitments match by exact table/partition identity, count and root independent of order or hints. Malformed/duplicate commitments refuse; a coherent difference signs an applied mismatch. After the one permitted signing call, a signer error or a nil-error result with blank replica ID or blank signature returns a zero attestation; receipt `ReplicaID` remains distinct from authenticated WORKER/current-use identity. The compound `SourceClaimRoot` is never compared to computed state. Candidate physical-byte equivalence, partition-delta proof and three-way promotion remain AC/AR/C5 gates, while an aborted receipt requires C3's separately authenticated committed abort transition.
...
| `accepted` | `<CA>/<ST>/<B>/<SS>/<SR>` | C3 committed acceptance/assignment plus durable original signed L3 statement and full reservation; bind input/read roots, exact original compact-JWS hash, grant/fence, source assignment, full predecessor pin and profile/activation history, then append any committed claim identity as a monotone set-once exact complete claim/root phase fact. Initial pre-claim ownership has neither claim field; accepted recovery never drops, replaces or conflicts with an existing claim. | Register/retain for the actual accepted source before initial execution and recover the same live A for same-operation recovery before exposing work; reservation ownership covers a commit-before-local-persist window until accepted ownership is established. | Exact matching applied/aborted terminal child for the predecessor/block, with publication/output or abort invariants, old-generation fencing, settled/quarantined writes and durable replay/cleanup/challenge successors; the local accepted operation must also be closed. |
| `replay` | `<CA>/<ST>/<B>/<SS>/<SR>/<ROLE>/<WORKER>` | C1/C3 authenticated committed job/assignment/history and original L3 statement plus this J/A; bind accepted identity, the required exact complete committed applied claim/root, full pin/O association, historical activation/pair and configured principal/role. | The trusted source/verifier wrapper registers and retains before any read/execution. Same live replay retry recovers A; each newly authorized historical replay gets a new A while old spent IDs stay spent. | This exact acquisition is closed/quiescent and its job/retention disposition is authenticated with surviving challenge/cleanup protection. A terminal job fact may support multiple separately closed acquisitions but cannot close an active one or borrow another A's close evidence. |
...
For B4 current use, the `accepted` class authorizes only its current actual source lifecycle under the trusted source assignment: initial pre-claim source execution enters B4 Prepare directly with both claim fields absent, while recovery preserves the same A and must carry any already committed exact claim/root without regaining pre-claim rights. `replay` authorizes a separately admitted source or verifier execution with the exact configured role/principal and complete committed applied claim/root; B5 remains applied-only and claim-required and is never the initial-source entry point. `challenge` protects obligations but requires a separate admitted replay acquisition to execute. `reservation`, `publication`, unknown, unbound and spent references cannot execute, and no class or phase fallback is allowed. The outer lifecycle registers/retains the exact acquisition before handing its unmodified reference to B4 or claim-required B5, while B4's invocation lease joins only that tracked invocation and never calls the durable `CloseUse` or local `Release`. A quiescence failure leaves that acquisition live/closing/uncertain for the concrete lifecycle owner; timeout, fixtures and GC cannot settle it.
