...
Challenge/debt transfer is durable before the last prior protection becomes spent, and a successor cannot disappear concurrently with handoff. Recovery preserves live/closing/uncertain ownership. Orphan registration protects or quarantines its known objects; a B1 live reference without authenticated registry state remains protected and cannot release; corruption or missing registry state never means absence. It may resume the same A only when the managing host/container authority proves the exact old process incarnation and all children stopped, or an enforced fence excludes every old operation. PID spelling, elapsed time, startup or a local counter is insufficient; absent proof retains the acquisition and refuses reclaim. GC independently requires no live/provisional/uncertain reference, registration/handoff, active lease or release-authorized-not-spent entry, plus authenticated retention-horizon authority, followed by a common-lock recheck.
...
Publication cancellation still needs a protected sealed-candidate lifecycle plus submission/publication exclusion that rules out late commits and settles unknown proposals. Current AR request-context cancellation stops only the RPC wait while an already submitted ApplyFuture continues, and current publication apply has no candidate cancellation fence; neither is cancellation authority. Challenge release still needs authenticated horizon/debt authority with monotone disposition or an enforced replacement fence. Current Finality, LastMergeable and cleanup absence are not artifact-horizon authority, and current OpenChallenge omits Safe/Rejected states; none proves finite reclamation. These are required future C1/C3/D3 implementation capabilities, not established records supplied by this document. The positive publication plus durable successor-retention branch remains supported. If existing authority cannot prove either missing predicate without a new replicated/public contract, return a bounded proposal before changing it; meanwhile retain/resume/quarantine rather than claim cancellation or eventual reclamation.
