...
### Errata (2026-07-06, post-implementation whole-branch review)

- **WorkSet shape.** As built, `AckedUnpublished []ManifestWork` is replaced by `NeedManifest bool` plus a separate `ManifestInputs() (ManifestInputs, bool)` read (parent snapshot id, safe block seq, assembled tables); and an eleventh class `ScheduledUnackedCleanups []CleanupWork` re-surfaces cleanups that committed `ScheduleUnsafeCleanup` but never received `RecordCleanupAck`, so a leader crash between schedule and SNode ack cannot strand the cleanup (the resend self-terminates once the `PromotedUnsafe` registry drains).
- **Manifest-debt gate (trust-chain fix).** `applyRecordPromotionIssued` rejects, and `WorkSet` stops advertising `PromotablePending`, while `safePrefix > SafeWatermark.SafeBlockSeq` (a newly-safe prefix whose manifest is unpublished — "manifest debt"). Without the gate, a block-(N+1) promotion issued before manifest(N) publishes could ack first and advance the live partition base + SafeParts that `ManifestInputs` reads, binding post-prefix state into a content-addressed manifest labeled `SafeBlockSeq=N` — corrupting the base that check 2 and P1c replay pin via `PrevSafeSnapshotID` (reachable via leader failover between issue and publish, or any publish retry window). During debt no promotion can be in flight (an unacked promotion holds its covered statements non-Safe, holding the prefix back), so `ManifestInputs` is exact by construction; the orchestrator additionally publishes before issuing in the same pass.
- **Promotion-ack contract (P1c-binding).** An `Applied=true` `RecordPromotionAck` must carry `Parts` mappings that exactly cover the promotion's candidate part hashes (bijection; duplicates and unselected hashes reject with zero state mutation). The SafePartMapping metadata feeds `SafeParts` → manifest `ActiveParts`, so the P1c SNode promotion executor MUST populate `Parts` for every applied ack or promotions fail closed.
- **Config cross-check.** `authority.private_key_hex` set with an empty `authority.allowed_addresses` is now a startup validation error (previously it surfaced only as per-retry rejection warnings at runtime).
...
| PromotablePending (per partition, no live pending promotion for it; **erratum 2026-07-06: suppressed while manifest debt is outstanding — see §3 errata**) | aggregate verified `PartRef`s → `PromoteSafePartition{PromotionSeq: current+1}` → `authority.Signer.SignPromotion` → propose `RecordPromotionIssued` → send `PromotionCommand` over the PromotionGateway stream. FSM rejects non-monotonic seq → re-read and retry (single-threaded loop makes races failover-only) |
| IssuedUnacked | re-send on SNode subscribe/reconnect and on the retry ticker (SNode dedups by promotion_seq watermark, §8.3) |
| AckedUnpublished (**erratum 2026-07-06: as built, `NeedManifest` + `ManifestInputs()`; handled BEFORE issuing new promotions in the same pass**) | assemble `SafeSnapshotManifest` (base = pinned previous manifest, updated partition roots from acks) → `Seal()` → propose `PublishSafeSnapshot` |
