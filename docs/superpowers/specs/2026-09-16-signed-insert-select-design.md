...
| A3 | Read pin, manifest root, state root, table schema, either profile id, network/shard or generation tampered; half-declared/genesis pin; correctly signed older query profile | Signature/admission/safe-chain checks reject each altered field. The FSM rejects a validly signed historical/weaker `query_profile_id` even when its binary is installed and `executor_profile_id` is current; either profile differing from the reservation/active pair is refused before execution. A self-consistent but unpublished manifest is rejected. An empty declared table is accepted; a missing table is rejected. |
...
| A13 | Mixed old/new agents, embedded servers, rewriters, source/verifiers and Arbiter; genesis activation, old → composite profile transition, query-profile downgrade, restart/migration/rollback with v3 history | Unsupported combinations refuse without ordinary-SQL fallback. The profile transition preserves schema/data, produces a new bound state/manifest, and the first new query chains from it using the activated pair. An old query profile remains usable for its authenticated historical block but cannot obtain a new reservation or pass a new submission, including after restart or a stale-agent retry. Old v2 JWS/L3/receipt vectors stay byte-identical; proto/JSON conformance and historical replay pass. |
