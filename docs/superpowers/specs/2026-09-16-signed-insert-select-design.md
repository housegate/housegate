...
After preparation, the relay checks the live generation, applies the detached result and resumes the remaining query hooks once. The cancellation/forwarding gate is checked again after continuation and durable forwarding intent, immediately before committing the one upstream Query attempt. Cancellation observed before that serialized transition forbids all upstream Query bytes and host Submit; a late worker/grant cannot reopen the generation. The gate is never held over RPC, FFI, fsync, hooks or network writes, so cancellation remains observable. If forwarding wins, a partial/lost write is indeterminate and the original operation must be reconciled at the host/sequencer; a subsequent Cancel does not prove Submit was prevented.

Persist request identity before acquire, canceled intent, the exact returned grant/envelope and ambiguous forwarding state. Cancellation before forwarding reconciles authenticated lookup by request identity, including draining and lost-acquire-response cases, and releases only an atomically proven-unconsumed matching generation. A negative lookup racing an in-flight acquire requires a committed request cancellation tombstone or equivalent authoritative fence; it is not sufficient to forget the request. Draining/granted release and absent-request cancellation persist released identities so late begin/grant cannot recreate canceled work. A consumed or indeterminate Submit retains durable recovery ownership under D9. Restart and reconnect cannot resurrect a canceled generation, reacquire it or re-sign it against another pin.
...
The proposed host `QueryOnlyPlan` is distinct from agent preparation: only independent executing-host validation may install it, and it owns local intake completion rather than forwarding signed SQL. Both plans are mutually exclusive with each other, deferred payload input, sample-based suppression and local abort-with-success. Host admission/analysis and intake run through the coordinator while the sole reader continues observing controls. Carry the same generation into the actual first Submit launch after durable intent; cancellation winning that gate forbids Submit and reconciles unconsumed reservation state, whereas a started/unknown or accepted Submit remains durably reconciled. Workers cannot independently write client terminals, release generations or promote candidates.
...
| A11 | Crash/lost response at request persistence, drain/acquire/grant, signing, cancellation/release, forwarding, Submit, restore, sort, capacity reservation, unsafe prepare, claim, publication and cleanup | Recovery keeps the same pin/id/output, resolves lost acquire/release responses by authenticated request identity and persistent tombstones, prevents late grants from forwarding, and releases only proven-unconsumed generations. Indeterminate forwarding/Submit/prepare and accepted work remain durably reconciled; no double-write or leaked/prematurely released capacity, and every late fenced claim rejects. |
