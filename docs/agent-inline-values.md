...
1. **Materialize.** The `materialize` plugin rewrites `now()`, `rand()`, `generateUUIDv4()` and the other supported nondeterministic functions to constant expressions. Enabling `inline_values` requires both `materialize.enabled` and `storage_integrity.agent.enabled` at startup. For this lane, only the recorded `applied` and `noop` outcomes are accepted; a missing, failed or unknown materialization outcome refuses the statement.
