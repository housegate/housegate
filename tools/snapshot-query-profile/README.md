...
Both outputs are fully generated and validated before writing. The CLI resolves relative, cleaned, symlinked, and existing-file identities and refuses output paths that alias each other, the recipe, or any measured artifact. It conservatively treats case-fold-equivalent basenames under the same resolved parent as aliases, including before either output exists. Each output is staged in its destination directory and renamed into place, so a profile path is never exposed with partial bytes.
