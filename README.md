...
- **Fail-open only without an SI surface.** With storage integrity disabled (`storage_integrity.enabled` false, the default when `storage_integrity.tables` is empty), a backend error or `UnsupportedStatement` falls back to the original SQL. Enabled storage integrity requires a contract-V2-capable backend (rewriter-go v0.13.0+ native, rewriter-grpc v0.15.0+) at startup and fails closed on every untrustworthy response.
